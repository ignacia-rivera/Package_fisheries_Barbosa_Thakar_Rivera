...
fish_function <- function(prices = NULL, catch = NULL, plot = FALSE){
  ...
  revenue <- dplyr::left_join(tidy_catches, prices, by = "fish") %>%
    dplyr::mutate(revenue = Catch * price) %>%
    dplyr::group_by(Site) %>%
    dplyr::summarize(Revenue = sum(revenue, na.rm = T))
  ...
  if(plot){
    plot_output <- ggplot2::ggplot(revenue, aes(x = Site, y = Revenue)) +
      ggplot2::geom_col(color = "blue", alpha = 0.25) +
